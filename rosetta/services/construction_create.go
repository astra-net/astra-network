...
func unpackWrappedTransactionFromString(
...
) (*WrappedTransaction, astraTypes.PoolTransaction, *types.Error) {
	...
	if wrappedTransaction.IsStaking {
		...
		intendedReceipt := &astraTypes.Receipt{
		...
		case stakingTypes.DirectiveDelegate:
			...
			validatorAddr, err := common2.ParseAddr(delegateMsg.ValidatorAddress)
			if err != nil {
				return nil, nil, common.NewError(common.InvalidTransactionConstructionError, map[string]interface{}{
					"message": err,
				})
			}
			delegatorAddr, err := common2.ParseAddr(delegateMsg.DelegatorAddress)
			if err != nil {
				return nil, nil, common.NewError(common.InvalidTransactionConstructionError, map[string]interface{}{
					"message": err,
				})
		...
		case stakingTypes.DirectiveUndelegate:
			...
			validatorAddr, err := common2.ParseAddr(undelegateMsg.ValidatorAddress)
			if err != nil {
				return nil, nil, common.NewError(common.InvalidTransactionConstructionError, map[string]interface{}{
					"message": err,
				})
			}
			delegatorAddr, err := common2.ParseAddr(undelegateMsg.DelegatorAddress)
			if err != nil {
				return nil, nil, common.NewError(common.InvalidTransactionConstructionError, map[string]interface{}{
					"message": err,
				})
		...
		plainTx := &astraTypes.Transaction{}
...
func (s *ConstructAPI) ConstructionPayloads(
	...
	if metadata.Transaction.FromShardID != nil && *metadata.Transaction.FromShardID != s.astra.ShardID {
		return nil, common.NewError(common.InvalidTransactionConstructionError, map[string]interface{}{
			"message": fmt.Sprintf("transaction is for shard %v != shard %v",
				*metadata.Transaction.FromShardID, s.astra.ShardID,
			),
		})
	}

	unsignedTx, rosettaError := ConstructTransaction(components, metadata, s.astra.ShardID)
	if rosettaError != nil {
		return nil, rosettaError
	}
...
func (s *ConstructAPI) getSigningPayload(
	tx astraTypes.PoolTransaction, senderAccountID *types.AccountIdentifier,
	...
	case *astraTypes.Transaction:
		payload.Bytes = s.signer.Hash(tx.(*astraTypes.Transaction)).Bytes()
	default:
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
