...
func NewNetworkAPI(astra *astra.Astra) server.NetworkAPIServicer {
	return &NetworkAPI{
		astra: astra,
	}
}
...
func (s *NetworkAPI) NetworkList(
	...
	network, err := common.GetNetwork(s.astra.ShardID)
	if err != nil {
...
func (s *NetworkAPI) NetworkStatus(
	...
	currBlock := s.astra.CurrentBlock()
	var currentHeader *block.Header
	var err error
	if currBlock.Number().Cmp(big.NewInt(0)) == 1 && !s.astra.IsStakingEpoch(currBlock.Epoch()) {
		...
		currentHeader, err = s.astra.HeaderByNumber(ctx, rpc.BlockNumber(blkNum.Uint64()))
	} else {
		currentHeader, err = s.astra.HeaderByNumber(ctx, rpc.LatestBlockNumber)
	}
	if err != nil {
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
	...
	genesisHeader, err := s.astra.HeaderByNumber(ctx, rpc.BlockNumber(0))
	if err != nil {
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
			"message": fmt.Sprintf("unable to get genesis header: %v", err.Error()),
		})
	}
	peers, rosettaError := getPeersFromNodePeerInfo(s.astra.GetPeerInfo())
	if rosettaError != nil {
		return nil, rosettaError
	}
	isSyncing, targetHeight, _ := s.astra.NodeAPI.SyncStatus(s.astra.BlockChain.ShardID())
	...
	if !nodeconfig.GetShardConfig(s.astra.ShardID).GetArchival() {
		maxGarbCollectedBlockNum := s.astra.BlockChain.GetMaxGarbageCollectedBlockNumber()
		if maxGarbCollectedBlockNum == -1 || maxGarbCollectedBlockNum >= currentHeader.Number().Int64() {
			...
			oldestBlockHeader, err := s.astra.HeaderByNumber(ctx, rpc.BlockNumber(maxGarbCollectedBlockNum+1))
			if err != nil {
				return nil, common.NewError(common.CatchAllError, map[string]interface{}{
...
func (s *NetworkAPI) NetworkOptions(
	...
	isArchival := nodeconfig.GetShardConfig(s.astra.ShardID).GetArchival()
