...
func TestGetShardingStructure(t *testing.T) {
	shardID := 0
	numShard := 4
	res := genShardingStructure(numShard, shardID, "http://s%d.t.astra.io:9500", "ws://s%d.t.astra.io:9800")
	...
	for i := 0; i < numShard; i++ {
		...
		if res[i]["http"].(string) != fmt.Sprintf("http://s%d.t.astra.io:9500", i) {
			t.Error("Error when generating sharding structure")
		}
		if res[i]["ws"].(string) != fmt.Sprintf("ws://s%d.t.astra.io:9800", i) {
			t.Error("Error when generating sharding structure")
		}
	}
}
