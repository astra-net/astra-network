...
function cleanup() {
   for pid in `/bin/ps -fu $USER| grep "astra\|soldier\|commander\|profiler\|beacon\|bootnode" | grep -v "grep" | grep -v "vi" | awk '{print $2}'`;
   do
...
function killnode() {
   ...
   if [ -n "port" ]; then
      pid=$(/bin/ps -fu $USER | grep "astra" | grep "$port" | awk '{print $2}')
...
ASTRA_OPT2="-bootnodes ${bn_ma}"

for i in 0{1..5} # {10..99}
do
    echo "launching new node $i ..."
    ($DRYRUN $ROOT/bin/astra -ip 127.0.0.1 -port 91$i -log_folder $log_folder -is_newnode $DB -account_index $i -min_peers $MIN $ASTRA_OPT $ASTRA_OPT2 $ASTRA_OPT3 -key /tmp/127.0.0.1-91$i.key 2>&1 | tee -a $LOG_FILE ) &
    sleep 5
done
