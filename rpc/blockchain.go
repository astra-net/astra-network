...
import (
	...
	internal_bls "github.com/astra-net/AstraNetwork/crypto/bls"
...
func NewPublicBlockchainAPI(astra *astra.Astra, version Version, limiterEnable bool, limit int) rpc.API {
...
func (s *PublicBlockchainService) ChainId(ctx context.Context) (interface{}, error) {
	...
	switch s.version {
	case V1:
		return hexutil.Uint64(s.astra.ChainID), nil
	case V2:
		return s.astra.ChainID, nil
	case Eth:
...
// Instead, users should send already signed raw transactions using astra_sendRawTransaction or eth_sendRawTransaction
...
func (s *PublicBlockchainService) BlockNumber(ctx context.Context) (interface{}, error) {
	// Fetch latest header
	header, err := s.astra.HeaderByNumber(ctx, rpc.LatestBlockNumber)
	if err != nil {
		return nil, err
	}
...
func (s *PublicBlockchainService) GetBlockByNumber(
	...
	if blockNumber.EthBlockNumber() == rpc.LatestBlockNumber {
		blockNum = s.astra.BlockChain.CurrentHeader().Number().Uint64()
	} else {
		blockNum = uint64(blockNumber.EthBlockNumber().Int64())
	}

	blk := s.astra.BlockChain.GetBlockByNumber(blockNum)
	...
	// Disable isBlockGreaterThanLatest checks for Ethereum RPC:s, but keep them in place for legacy astra_ RPC:s for now to ensure backwards compatibility
	if blk == nil {
		DoMetricRPCQueryInfo(GetBlockByNumber, FailedNumber)
		if s.version == Eth {
			return nil, nil
...
func (s *PublicBlockchainService) GetBlockByHash(
	...
	// Fetch the block
	blk, err := s.astra.GetBlock(ctx, blockHash)
	if err != nil || blk == nil {
		DoMetricRPCQueryInfo(GetBlockByHash, FailedNumber)
		return nil, err
	}
...
func (s *PublicBlockchainService) GetBlocks(
	...
	if blockNumberEnd.EthBlockNumber() == rpc.LatestBlockNumber {
		blockEnd = s.astra.BlockChain.CurrentHeader().Number().Int64()
	}
	...
	for i := blockStart; i <= blockEnd; i++ {
		...
		if blockNum.Int64() > s.astra.CurrentBlock().Number().Int64() {
			break
		}
...
func (s *PublicBlockchainService) GetBlockSigners(
	...
	if blockNum.Int64() == 0 || blockNum.Int64() >= s.astra.CurrentBlock().Number().Int64() {
		return []string{}, nil
	}
	...
	blk := s.astra.BlockChain.GetBlockByNumber(bn)
	if blk == nil {
...
func (s *PublicBlockchainService) GetBlockReceipts(
	...
	block, err := s.astra.GetBlock(ctx, blockHash)
	if err != nil {
		return nil, err
	}

	receipts, err := s.astra.GetReceipts(ctx, blockHash)
	if err != nil {
		return nil, err
	}
...
func (s *PublicBlockchainService) IsBlockSigner(
	...
	if blockNum == rpc.PendingBlockNumber {
		...
		bn = s.astra.BlockChain.CurrentBlock().NumberU64()
	} else {
		bn = uint64(blockNum.Int64())
	}
...
func (s *PublicBlockchainService) GetSignedBlocks(
	...
	curEpoch := s.astra.CurrentBlock().Epoch()
	var totalSigned uint64
	if !s.astra.ChainConfig().IsStaking(curEpoch) {
		// calculate signed before staking epoch
		totalSigned := uint64(0)
		lastBlock := uint64(0)
		blockHeight := s.astra.CurrentBlock().Number().Uint64()
		instance := shard.Schedule.InstanceForEpoch(s.astra.CurrentBlock().Epoch())
		...
		curVal, err := s.astra.BlockChain.ReadValidatorInformation(ethAddr)
		if err != nil {
			return nil, err
		}
		prevVal, err := s.astra.BlockChain.ReadValidatorSnapshot(ethAddr)
		if err != nil {
			return nil, err
		}
...
func (s *PublicBlockchainService) GetLeader(ctx context.Context) (string, error) {
	...
	// Fetch Header
	blk := s.astra.BlockChain.CurrentBlock()
...
func (s *PublicBlockchainService) GetShardingStructure(
	...
	epoch := s.astra.CurrentBlock().Epoch()
...
func (s *PublicBlockchainService) LatestHeader(ctx context.Context) (StructuredResponse, error) {
	...
	// Response output is the same for all versions
	leader := s.astra.GetLeaderAddress(header.Coinbase(), header.Epoch())
...
func (s *PublicBlockchainService) GetLatestChainHeaders(
	...
	return NewStructuredResponse(s.astra.GetLatestChainHeaders())
}
...
func (s *PublicBlockchainService) GetLastCrossLinks(
	...
	// Fetch crosslinks
	crossLinks, err := s.astra.GetLastCrossLinks()
	if err != nil {
		DoMetricRPCQueryInfo(GetLastCrossLinks, FailedNumber)
		return nil, err
	}
...
func (s *PublicBlockchainService) GetHeaderByNumber(
	...
	if s.version != Eth && isBlockGreaterThanLatest(s.astra, blockNum) {
		DoMetricRPCQueryInfo(GetHeaderByNumber, FailedNumber)
		return nil, ErrRequestedBlockTooHigh
	}

	// Fetch Header
	header, err := s.astra.HeaderByNumber(ctx, blockNum)
...
func (s *PublicBlockchainService) GetProof(
	...
	state, err := s.astra.BeaconChain.StateAt(header.Root())
...
func (s *PublicBlockchainService) GetCurrentUtilityMetrics(
	...
	// Fetch metrics
	metrics, err := s.astra.GetCurrentUtilityMetrics()
	if err != nil {
		DoMetricRPCQueryInfo(GetCurrentUtilityMetrics, FailedNumber)
		return nil, err
	}
...
func (s *PublicBlockchainService) GetSuperCommittees(
	...
	cmt, err := s.astra.GetSuperCommittees()
	if err != nil {
		DoMetricRPCQueryInfo(GetSuperCommittees, FailedNumber)
		return nil, err
	}
...
func (s *PublicBlockchainService) GetCurrentBadBlocks(
	...
	for _, blk := range s.astra.GetCurrentBadBlocks() {
		// Response output is the same for all versions
...
func (s *PublicBlockchainService) GetTotalSupply(
	...
	return stakingReward.GetTotalTokens(s.astra.BlockChain)
}
...
func (s *PublicBlockchainService) GetCirculatingSupply(
	...
	return chain.GetCirculatingSupply(s.astra.BlockChain)
}
...
func (s *PublicBlockchainService) GetStakingNetworkInfo(
	...
	totalStaking := s.astra.GetTotalStakingSnapshot()
	header, err := s.astra.HeaderByNumber(ctx, rpc.LatestBlockNumber)
	if err != nil {
		DoMetricRPCQueryInfo(GetStakingNetworkInfo, FailedNumber)
		return nil, err
	}
	medianSnapshot, err := s.astra.GetMedianRawStakeSnapshot()
	if err != nil {
		DoMetricRPCQueryInfo(GetStakingNetworkInfo, FailedNumber)
		return nil, err
	}
...
func (s *PublicBlockchainService) InSync(ctx context.Context) (bool, error) {
	...
	inSync, _, diff := s.astra.NodeAPI.SyncStatus(s.astra.BlockChain.ShardID())
...
func (s *PublicBlockchainService) BeaconInSync(ctx context.Context) (bool, error) {
	...
	inSync, _, diff := s.astra.NodeAPI.SyncStatus(s.astra.BeaconChain.ShardID())
	if !inSync && diff <= inSyncTolerance {
		inSync = true
	}
	return inSync, nil
...
func isBlockGreaterThanLatest(astra *astra.Astra, blockNum rpc.BlockNumber) bool {
	...
	return uint64(blockNum) > astra.CurrentBlock().NumberU64()
}
...
func (s *PublicBlockchainService) SetNodeToBackupMode(ctx context.Context, isBackup bool) (bool, error) {
	...
	return s.astra.NodeAPI.SetNodeBackupMode(isBackup), nil
}
...
func (helper *bcServiceHelper) GetLeader(b *types.Block) string {
	...
	leader := helper.astra.GetLeaderAddress(b.Coinbase(), b.Epoch())
...
func (helper *bcServiceHelper) getSignerData(bn uint64) (*signerData, error) {
	...
	sd, err := getSignerData(helper.astra, bn)
	if err != nil {
...
func getSignerData(astra *astra.Astra, number uint64) (*signerData, error) {
	slots, mask, err := astra.GetBlockSigners(context.Background(), rpc.BlockNumber(number))
	if err != nil {
		return nil, err
	}
