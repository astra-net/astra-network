...
func NewPublicTransactionAPI(astra *astra.Astra, version Version) rpc.API {
	return rpc.API{
		Namespace: version.Namespace(),
		Version:   APIVersion,
		Service:   &PublicTransactionService{astra, version},
		Public:    true,
	}
}
...
func (s *PublicTransactionService) GetAccountNonce(
	...
	return s.astra.GetAccountNonce(ctx, addr, blockNum)
}
...
func (s *PublicTransactionService) GetTransactionsCount(
	...
	// Response output is the same for all versions
	return s.astra.GetTransactionsCount(address, txType)
}
...
func (s *PublicTransactionService) GetStakingTransactionsCount(
	...
	// Response output is the same for all versions
	return s.astra.GetStakingTransactionsCount(address, txType)
}
...
func (s *PublicTransactionService) GetTransactionByHash(
	...
	if tx == nil {
		...
		if tx := s.astra.TxPool.Get(hash); tx != nil {
...
func (s *PublicTransactionService) GetStakingTransactionByHash(
	...
	// Try to return an already finalized transaction
	stx, blockHash, blockNumber, index := rawdb.ReadStakingTransaction(s.astra.ChainDb(), hash)
	if stx == nil {
		// Try to return a pending transaction
		if tx := s.astra.TxPool.Get(hash); tx != nil {
...
func (s *PublicTransactionService) GetTransactionsHistory(
	...
	address = addr.Hex()
	hashes, err := s.astra.GetTransactionsHistory(address, args.TxType, args.Order)
	if err != nil {
		DoMetricRPCQueryInfo(GetTransactionsHistory, FailedNumber)
		return nil, err
	}
...
func (s *PublicTransactionService) GetStakingTransactionsHistory(
	...
	address = addr.Hex()
	hashes, err := s.astra.GetStakingTransactionsHistory(address, args.TxType, args.Order)
	if err != nil {
		utils.Logger().Debug().
			Err(err).
...
func (s *PublicTransactionService) GetBlockTransactionCountByNumber(
	...
	// Fetch block
	block, err := s.astra.BlockByNumber(ctx, blockNum)
	if err != nil {
		utils.Logger().Debug().
			Err(err).
...
func (s *PublicTransactionService) GetTransactionReceipt(
	...
	if tx == nil {
		...
		if block, _ := s.astra.GetBlock(ctx, blockHash); block != nil {
...
func (s *PublicTransactionService) GetCXReceiptByHash(
	...
	if cx, blockHash, blockNumber, _ := rawdb.ReadCXReceipt(s.astra.ChainDb(), hash); cx != nil {
		// Format response according to version
		switch s.version {
		case V1, Eth:
...
func (s *PublicTransactionService) ResendCx(ctx context.Context, txID common.Hash) (bool, error) {
	...
	_, success := s.astra.ResendCx(ctx, txID)
