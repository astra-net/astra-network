...
func migrateConf(confBytes []byte) (astraconfig.AstraConfig, string, error) {
	var (
		migratedFrom string
	)
	confTree, err := toml.LoadBytes(confBytes)
	if err != nil {
		return astraconfig.AstraConfig{}, "", fmt.Errorf("config file parse error - %s", err.Error())
	}
	confVersion, found := confTree.Get("Version").(string)
	if !found {
		return astraconfig.AstraConfig{}, "", errors.New("config file invalid - no version entry found")
	}
	...
	var config astraconfig.AstraConfig
