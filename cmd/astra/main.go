...
import (
	...
	"github.com/astra-net/astra-network/api/service/pprof"
	"github.com/astra-net/astra-network/api/service/prometheus"
	"github.com/astra-net/astra-network/api/service/synchronize"
	"github.com/astra-net/astra-network/astra/downloader"
	"github.com/astra-net/astra-network/common/fdlimit"
	"github.com/astra-net/astra-network/common/ntp"
...
var rootCmd = &cobra.Command{
	Use:   "astra",
	Short: "astra is the Astra node binary file",
	Long: `astra is the Astra node binary file
...
# start a validator node with customized bls key folder
    ./astra --bls.dir [bls_folder]

# start a validator node with open RPC endpoints and customized ports
    ./astra --http.ip=0.0.0.0 --http.port=[http_port] --ws.ip=0.0.0.0 --ws.port=[ws_port]

# start an explorer node
    ./astra --run=explorer --run.shard=[shard_id]

# start a astra internal node on testnet
    ./astra --run.legacy --network testnet
`,
	Run: runAstraNode,
}
...
func runAstraNode(cmd *cobra.Command, args []string) {
...
func getAstraConfig(cmd *cobra.Command) (astraconfig.AstraConfig, error) {
	var (
		config         astraconfig.AstraConfig
		err            error
		migratedFrom   string
		configFile     string
		isUsingDefault bool
	...
	if err != nil {
		return astraconfig.AstraConfig{}, err
	}
		...
		if stat.Mode()&os.ModeCharDevice != 0 {
			if promptConfigUpdate() {
				...
				if err != nil {
					...
					fmt.Println("Update config manually with `./astra config update [config_file]`")
				}
			}

		} else {
			fmt.Println("Update saved config with `./astra config update [config_file]`")
		}
	}

	applyRootFlags(cmd, &config)

	if err := validateAstraConfig(config); err != nil {
		return astraconfig.AstraConfig{}, err
	}
	sanityFixAstraConfig(&config)
	return config, nil
}

func applyRootFlags(cmd *cobra.Command, config *astraconfig.AstraConfig) {
...
func setupNodeLog(config astraconfig.AstraConfig) {
...
func setupNodeAndRun(hc astraconfig.AstraConfig) {
	var err error

	nodeconfigSetShardSchedule(hc)
	nodeconfig.SetShardingSchedule(shard.Schedule)
	nodeconfig.SetVersion(getAstraVersion())
...
func nodeconfigSetShardSchedule(config astraconfig.AstraConfig) {
	...
	case nodeconfig.Devnet:
		var dnConfig astraconfig.DevnetConfig
...
func setupLegacyNodeAccount(hc astraconfig.AstraConfig) error {
...
func setupStakingNodeAccount(hc astraconfig.AstraConfig) error {
...
func createGlobalConfig(hc astraconfig.AstraConfig) (*nodeconfig.ConfigType, error) {
	var err error
...
func setupConsensusAndNode(hc astraconfig.AstraConfig, nodeConfig *nodeconfig.ConfigType) *node.Node {
	// Consensus object.
	...
	// Parse minPeers from astraconfig.AstraConfig
	var minPeers int
...
func processNodeType(hc astraconfig.AstraConfig, currentNode *node.Node, currentConsensus *consensus.Consensus) {
...
func setupPprofService(node *node.Node, hc astraconfig.AstraConfig) {
...
func setupPrometheusService(node *node.Node, hc astraconfig.AstraConfig, sid uint32) {
...
func setupSyncService(node *node.Node, host p2p.Host, hc astraconfig.AstraConfig) {
...
func setupBlacklist(hc astraconfig.AstraConfig) (map[ethCommon.Address]struct{}, error) {
	...
	for _, line := range strings.Split(string(dat), "\n") {
		if len(line) != 0 { // blacklist file may have trailing empty string line
			adr := strings.TrimSpace(strings.Split(string(line), "#")[0])
			addr, err := common.ParseAddr(adr)
			if err != nil {
				return nil, err
			}
			addrMap[addr] = struct{}{}
...
func setupLocalAccounts(hc astraconfig.AstraConfig, blacklist map[ethCommon.Address]struct{}) ([]ethCommon.Address, error) {
	...
	for _, line := range lines {
		if len(line) != 0 { // the file may have trailing empty string line
			...
			addr, err := common.ParseAddr(trimmedLine)
			if err != nil {
				return nil, err
			}
