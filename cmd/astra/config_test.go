...
[Network]
  BootNodes = ["/dnsaddr/bootstrap.m.astranetwork.com"]
  DNSPort = 9000
  DNSZone = "m.astranetwork.com"
  LegacySyncing = false
  NetworkType = "mainnet"
...
[WS]
  ...
  Port = 9800`
	...
	config, migratedFrom, err := loadAstraConfig(file)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPersistConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			config: makeTestConfig("mainnet", func(cfg *astraconfig.AstraConfig) {
				...
				webHook := "web hook"
				cfg.Legacy = &astraconfig.LegacyConfig{
	...
	for i, test := range tests {
		...
		if err := writeAstraConfigToFile(test.config, file); err != nil {
			t.Fatal(err)
		}
		config, _, err := loadAstraConfig(file)
		if err != nil {
			t.Fatal(err)
		}
