...
func validateAstraConfig(config astraconfig.AstraConfig) error {
	var accepts []string
...
func sanityFixAstraConfig(hc *astraconfig.AstraConfig) {
...
func getDefaultDNSSyncConfig(nt nodeconfig.NetworkType) astraconfig.DnsSync {
...
func getDefaultNetworkConfig(nt nodeconfig.NetworkType) astraconfig.NetworkConfig {
	bn := nodeconfig.GetDefaultBootNodes(nt)
	return astraconfig.NetworkConfig{
...
func getDefaultSyncConfig(nt nodeconfig.NetworkType) astraconfig.SyncConfig {
	switch nt {
	case nodeconfig.Mainnet:
...
var dumpConfigCmd = &cobra.Command{
	...
	Short: "dump default config file for astra binary configurations",
	Long:  "dump default config file for astra binary configurations",
	Args:  cobra.MinimumNArgs(1),
	Run:   dumpConfig,
}
...
func promptConfigUpdate() bool {
	...
	select {
	case <-timeoutTimer.C:
		fmt.Println("Timed out - update manually with ./astra config update [config_file]")
		return false
	case <-read:
...
func loadAstraConfig(file string) (astraconfig.AstraConfig, string, error) {
...
func updateConfigFile(file string) error {
	...
	if err := writeAstraConfigToFile(config, file); err != nil {
		return err
	}
...
func writeAstraConfigToFile(config astraconfig.AstraConfig, file string) error {
