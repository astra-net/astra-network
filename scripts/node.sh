...
get_version() {
    if [[ -n ${ASTRA_RELEASE:-} ]]; then
        ...
        astra_rel=$(curl -f -s https://astranetwork.com/releases/latest) || (color "31" "Get latest version failed. You may manually download the binaries from Github release page. https://github.com/harmony-one/astra/releases/" && exit 1)
        readonly astra_rel
    fi
}
