...
while getopts "lbvp" OPTION; do
	...
	b)
		NETWORK="betanet"
		declare -A PORT=( [POST]="http://s0.t.astranetwork.com:9500/" [GET]="http://e0.t.astranetwork.com:5000/" )
		BLOCK_0_HASH=$(curl --location --request POST "http://l0.t.astranetwork.com:9500" \
		...
		TRANSACTION_HASH=$(curl  --location --request POST "http://l0.t.astranetwork.com:9500" \
			  --header "Content-Type: application/json" \
			    --data "{\"jsonrpc\":\"2.0\",\"method\":\"astra_sendRawTransaction\",\"params\":[\""$SIGNED_RAW_TRANSACTION"\"],\"id\":1}" | jq -r '.result')
		echo "TRANSACTION_HASH:"
		echo $TRANSACTION_HASH
		sleep 20s
		TRANSACTION=$(curl --location --request POST "http://l0.t.astranetwork.com:9500" \
			  --header "Content-Type: application/json" \
...
if [ "$NETWORK" == "localnet" ]; then
	...
	POSTDATA[astra_getBalance]="astra_getBalance\",\"params\":[\"0x3aea49553Ce2E478f1c0c5ACC304a84F5F4d1f98\", \"latest\"]"
