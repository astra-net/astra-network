...
SET SELF=%0
SET DOCKER_IMAGE=astranetwork/node:s3
...
FOR /F "tokens=* USEBACKQ" %%F IN (`docker ps -q -a -f "name=^astra-%account_id%-%port_base%$"`) DO (
	SET running_container=%%F
)

IF NOT "%running_container%"=="" (
    ECHO Stop node for account id: %account_id% port %port_base%
	docker rm -v -f astra-%account_id%-%port_base% >NUL
)
...
docker run -it -d ^
	--name astra-%account_id%-%port_base% ^
	-p %port_base%:%port_base% -p %port_rest%:%port_rest% -p %port_rpc%:%port_rpc% ^
	-e NODE_PORT=%port_base% ^
	-e NODE_ACCOUNT_ID=%account_id% ^
	--mount type=volume,source=db-%account_id%-%port_base%,destination=/astra/db ^
	--mount type=volume,source=log-%account_id%-%port_base%,destination=/astra/log ^
	%DOCKER_IMAGE% >NUL

echo.
echo ======================================
echo Node for account %account_id% (port %port_base%) is running in container 'astra-%account_id%-%port_base%'"
echo.
echo To check console log, please run `docker logs -f astra-%account_id%-%port_base%`"
