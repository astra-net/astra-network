...
if [ -n "$(docker ps -q -a -f name=^astra-${tag}-${port_base}$)" ]; then
  echo "Stop node for tag: $tag, port: $port_base"
  docker rm -v -f astra-${tag}-${port_base} >/dev/null
...
mkdir -p ${db_dir}/astra_db_0
mkdir -p ${db_dir}/astra_db_1
mkdir -p ${db_dir}/astra_db_2
mkdir -p ${db_dir}/astra_db_3

docker run -it -d \
  --name astra-$tag-$port_base \
  ...
  -v $(realpath ${db_dir}/astra_db_0):/astra/astra_db_0 \
...
echo
echo "======================================"
echo "Node for tag ($tag) (port $port_base) is running in container 'astra-$tag-$port_base'"
echo
echo "To check console log, please run \`docker logs -f astra-$tag-$port_base\`"
