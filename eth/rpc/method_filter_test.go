...
func TestRpcMethodFilter(t *testing.T) {
	method_filters_toml := `
		Allow = [ 
			"astra_method1",
			"wildcard:astrav2_method?",
			"eth*",
			"astra_getNetworkInfo",
			"regex:^astra_send[a-zA-Z]+"
		]
	...
	tests := []struct {
		...
		0:  {"astra_method1", true},         // auto detected exact match which exists (case-insensitive)
		1:  {"astra_MeThoD1", true},         // check case-insensitive
		2:  {"astra_method2", false},        // not exist in allows
		3:  {"astrav2_method5", true},       // wildcard
		4:  {"astrav2_method", false},       // false case for wild card
		...
		7:  {"astra_getStakingInfo", false}, // deny wild card
		8:  {"abc", false},                  // not exist pattern
		9:  {"astra_getNetworkInfo", false}, // case-insensitive normal word match
		10: {"astra_sendTx", false},         // exact match (case-sensitive)
		11: {"astra_sendtx", true},          // exact match (case-sensitive)
	}
...
func TestRpcMethodAllowAllFilter(t *testing.T) {
	...
	tests := []struct {
		...
		0: {"mtd1", false},
		1: {"astra_method1", false},
		2: {"astrav2_method5", true},
		3: {"astrav2_method", true},
		4: {"eth_chainID", true},
		5: {"eth_getValidator", false},
		6: {"astra_getStakingInfo", false},
		7: {"abc", true},
		8: {"astra_getStakingNetworkInfo", false},
	}
...
func TestRpcMethodDenyAllFilter(t *testing.T) {
	...
	tests := []struct {
		...
		0: {"mtd1", false},
		1: {"astra_method1", false},
		2: {"astrav2_method5", false},
		3: {"astrav2_method", false},
		4: {"eth_chainID", false},
		5: {"eth_getValidator", false},
		6: {"astra_getStakingInfo", false},
		7: {"abc", false},
...
func TestEmptyRpcMethodFilter(t *testing.T) {
	...
	tests := []struct {
		...
		0: {"astra_method1", true},
		1: {"astra_method2", true},
		2: {"astrav2_method5", true},
		3: {"astrav2_method", true},
		4: {"eth_chainID", true},
		5: {"eth_getValidator", true},
		6: {"astra_getStakingInfo", true},
		7: {"abc", true},
		8: {"astra_getNetworkInfo", true},
	}
